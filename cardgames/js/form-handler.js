...
// js/form-handler.js - Phần preparePayloadForSheet() cần giữ nguyên
...
preparePayloadForSheet() {
    ...
    if (CONFIG.isMatrixDeck()) {
        ...
        // Tạo dữ liệu cho 15 cột Matrix_R0C0 -> Matrix_R2C4
        for (let row = 0; row <= 2; row++) {
            for (let col = 0; col <= 4; col++) {
                const cellKey = `Matrix_R${row}C${col}`;
                payload[cellKey] = '';
            }
        }
        ...
        matrixCards.forEach(card => {
            const cellKey = `Matrix_R${card.row}C${card.col}`;
            if (payload[cellKey] === '') {
                payload[cellKey] = card.name;
            } else {
                payload[cellKey] += ', ' + card.name;
            }
        });
        
        // Thêm metadata cho việc hiển thị preview
        payload.matrixData = {};
        for (let row = 0; row <= 2; row++) {
            for (let col = 0; col <= 4; col++) {
                const cellKey = `Matrix_R${row}C${col}`;
                payload.matrixData[`row${row}_col${col}`] = payload[cellKey] ? payload[cellKey].split(', ') : [];
            }
        }
        payload.matrixCardsCount = matrixCards.length;
        
        // Tạo Level_0 -> Level_4 rỗng để tránh lỗi
        for (let i = 0; i <= 4; i++) {
            payload[`Level_${i}`] = '';
        }
    ...
    // ===== XỬ LÝ CHO CÁC BỘ THƯỜNG =====
    const currentLevels = CONFIG.getCurrentLevels();
    const placedCards = GameState.getPlacedCards();
    
    // Khởi tạo Level_0 -> Level_4
    for (let i = 0; i <= 4; i++) {
        payload[`Level_${i}`] = '';
    }
    
    // Gán thẻ vào đúng level
    placedCards.forEach(card => {
        if (card.level !== undefined && card.level >= 0 && card.level <= 4) {
            const columnKey = `Level_${card.level}`;
            if (payload[columnKey] === '') {
                payload[columnKey] = card.name;
            } else {
                payload[columnKey] += ', ' + card.name;
            }
        }
    ...
    for (let row = 0; row <= 2; row++) {
        for (let col = 0; col <= 4; col++) {
            payload[`Matrix_R${row}C${col}`] = '';
        }
    }
    
    payload.totalCardsPlaced = placedCards.length;
    
    return payload;
},

// Hàm showSuccessWithPreview vẫn giữ nguyên (không thay đổi)
showSuccessWithPreview(data, currentLevels = null) {
    ...
    if (data.matrixData) {
        ...
        for (let i = 0; i <= 4; i++) {
            const levelData = data[`Level_${i}`];
            if (levelData && levelData !== '') {
                preview += `
                    <div style="background: #f5f5f5; padding: 10px; border-radius: 8px; margin-bottom: 8px;">
                        <strong>Level_${i}:</strong><br>
                        <span style="font-size: 12px;">${Utils.escapeHtml(levelData.substring(0, 200))}${levelData.length > 200 ? '...' : ''}</span>
                    </div>
                `;
            }
        }
        ...
        preview += `
            <div style="margin-top: 12px; padding: 8px; background: #f5f5f5; border-radius: 8px;">
                <strong>📦 Thống kê:</strong> Đã xếp ${data.totalCardsPlaced || 0} thẻ
            </div>
        `;
    }
