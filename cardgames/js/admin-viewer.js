...
async showDetail(index) {
    ...
    // 3. NẾU LÀ BỘ DISC, THÊM PHÂN TÍCH VÀO CUỐI
    if (deckName === 'disc') {
        const discAnalysis = this.calculateDiscAnalysis(response);
        if (discAnalysis) {
            const discHtml = this.renderDiscAnalysis(discAnalysis);
            html += discHtml;  // THÊM VÀO CUỐI, không phải đầu
        }
    }
    ...
    attachCardHoverEvents() {
        // Tìm tất cả các thẻ trong modal
        const cardItems = document.querySelectorAll('#modalBody .column-card-item');
    ...
    handleCardMouseEnter(event) {
        ...
        const deckName = cardItem.getAttribute('data-deck-name');
